...

{
	...
	private static int selectionLettres(int niveau) throws IOException// renvoie x = le nombre d'erreur (nbre de lettres donnees qui ne
													// sont pas dans le mot) ou -1 si 0 erreur
	{
		...
		char[] joueur = new char[mot.length]; // tableau que voit le joueur pendant sa partie, les lettres justes sont
												// affichees, des 0 la ou la lettre n'est pas connu, de la taille du mot
		boolean[] erreur = new boolean[alph.length]; // tableau de boolean qui est de meme taille de alph, ex: sa case 4
														// vaut True si lettre deja mise/proposee et False si non

		mot = ChoixMot(niveau); // recupere le mot de la fonction ChoixMot (qui est mis sous forme de tableau de
								// caractere)
		...
		while ((n > 0) || (Arrays.equals(mot, joueur) == x)) { //tant que ttes les erreurs sont pas faites ou que les deux tableaux sont pas égaux, on continue
			System.out.println("Saisir une lettre"); // penser a essayer en majuscule et minuscule pour voir si les deux
														// marchent
			l = flux.readLine().charAt(0);
			
			for (i = 0; i < n_err; i += 1) // permet de dire a deja ete mise ou non, true si deja mise, false sinon, et
											// on met a true en rentrant la lettre si elle n'avait pas deja ete rentree
											// avant
			{
				if (alph[i] == l) {
					if (erreur[i] == true) {
			...
			for (j = 0; j < n_m; j += 1) // si la lettre n'a pas deja ete donnee, il cherche dans le mot si elle y est,
											// et si elle y est il la met dans le tableau joueur, sinon il compte une
											// erreur
			{
		...
		System.out.println("Jack le pendu est mort...."); // fin de la boucle while = il a depasse son nombre d'essai
															// possible (soit 11), donc message de fin
	...
	public static void main(String[] arg) throws IOException // je sais plus comment on ecrit un programme principal en
																// java
	{
		int niveau, erreurs;
		...
		System.out.println(
			" Il etait une fois, un homme repondant au nom de Jack injustement condamne a la pendaison pour vol... Sa pendaison a lieu aujourd hui, mais vous pouvez le sauver ! Pour cela, vous devez trouver le mot secret... Mais attention, vous n avez droit qu'a 11 erreurs ou il finira pendu."); // mettre
			// le
			// blabla
			// du
			// debut
			...
			{
				System.out.println(
					"Il faut mettre 1 pour le niveau facile, 2 pour un niveau moyen ou 3 pour le niveau difficile. Et rien d'autre fartasse");
		...
		erreurs = selectionLettres(niveau);// j'ai rajoute le erreurs = pour pouvoir choper le nbr d'erreurs, puis on
											// recupere le nombre d'erreurs faites et mtn il faut les mettre dans le
											// fichier pour faire le podium jsp comment
		...
		if (erreurs == 11)// ici on indique si gagne ou perdu en fonction du nbr d'erreurs dcp
		{
			...
			ligne = (nom + " " + "a gagne en faisant" + " " + erreurs + " " + "erreurs");// on cree la ligne pr rentrer
																							// dans le fichier
		}
