...
public class jeuGraphev15 {
			...
			if ((cond1) && (cond2)) {
				...
				switch (choix) {
					...
					case 2: {
						...
						break;
					default:
						System.out.println("saisie erronee choisir 1 pour se focus sur le present et 2 sur le futur");
				...
				if ((cond1) && (cond2)) {
					...
					switch (choix) {
						...
						case 2: {
							...
							break;
						default:
							System.out.println("saisie erronee choisir 1 pour se focus sur le passe et 2 sur le futur");
							choixFocus(f, donnees);
					}
				} else {
					if (cond1) {
			...
			} else // f=2
			{
				...
				if ((cond1) && (cond2)) {
					...
					switch (choix) {
						...
						default:
							System.out
									.println("saisie erronee choisir 1 pour se focus sur le passe et 2 sur le present");
							choixFocus(f, donnees);
					}
				} else {
					if (cond1) {
	...
	private static void confForfait(int[][] LP, String[][][] Mj, String[][][] Ms, Tdonnee[] donnees, String[] JR)
			throws IOException // fct compile
	{
		int conf;
		...
		switch (conf) {
			...
			case 1: {
				...
				break;
			default:
				System.out.println("saisie erronee taper 0 pour confirmer le forfait ou 1 pour revenir en arriere "); // fct
																														// confirmer
	...
	private static void confEgalite(int[][] LP, String[][][] Mj, String[][][] Ms, Tdonnee[] donnees, String[] JR)
			throws IOException // fct compile
	{
		int conf;
		conf = Integer.valueOf(flux.readLine()).intValue();
		switch (conf) {
			...
			case 1: {
				...
				break;
			default:
				System.out.println("saisie erronee taper 0 pour confirmer l'egalite ou 1 pour revenir en arriere ");
	...
	private static void finTour(int[][] LP, String[][][] Mj, String[][][] Ms, Tdonnee[] donnees, String[] JR)
			throws IOException // fct compiler sauvegarde de partie ; et a tester
	{
		int choix, k;
		...
		switch (choix) {
			...
			case 2: {
				...
				break;
			case 3: {
				System.out.println("taper 0 pour confirmer l'egalite ou 1 pour revenir en arriere");// entree et retour
																									// arriere
				confEgalite(LP, Mj, Ms, donnees, JR);
			}
				break;
			case 4: {
				...
				break;
			default:
				System.out.println("saisie erronee tapez: ");
				finTour(LP, Mj, Ms, donnees, JR);
		}
	}
	// =======================================================================================================

	private static void sauvExiste(int[][] LP, String[] JR, String[][][] Mj, String[][][] Ms, Tdonnee[] donnees)
			throws IOException {
		String sauvegarde, nom;
	...
	private static void confSauv(String sauvegarde, String nom, int LP[][], String[] JR, String[][][] Mj,
			String[][][] Ms, Tdonnee[] donnees) throws IOException {
		int choix;
	...
	private static int choixPion(int n, int m, int LP[][], String[] JR, String[][][] Mj, String[][][] Ms)
			throws IOException // fct compiler m c est f
	{
		int increment, i, j, pactif, k;
		String lettre, choix;
		lettre = "azertyui";
		increment = 0;
		for (i = 1; i < 5; i += 1) // peut etre opti avec un while lie au nombre dans ere et increment et en
									// variant i j de +1 par boucle
		{
			for (j = 1; j < 5; j += 1) {
	...
	private static void choixAction(int m, int i, int j, String[] JR, String[][][] M, Tdonnee[] donnees, boolean cond1,
			boolean cond2) throws IOException// fct compile
	{
		int choix;
		choix = Integer.valueOf(flux.readLine()).intValue();
		switch (choix) {
			...
			case 2: // TP
			{
				...
				break;
			default:
				System.out.println("saisie erronee tapez 1 pour un deplacement et 2 pour un voyage temporel");
	...
	private static void choixDeplacement(String[] JR, String[][][] Mj, Tdonnee[] donnees) throws IOException // fct
																												// compiler
	{
		int dir;
		if (iactif != 1) {
	...
	private static void elim(int i, int j, int m, String[] JR, String[][][] M, Tdonnee[] donnees) throws IOException // fct
																														// compiler
	{
		int jrPion;
		jrPion = 0;
	...
	private static void deplacementHaut(int i, int j, int m, String[] JR, String[][][] M, Tdonnee[] donnees)
			throws IOException // fct compiler
	{
		String c;
	...
	private static void collisionHaut(int i, int j, int m, String[] JR, String[][][] M, Tdonnee[] donnees)
			throws IOException // fct compiler
	{
	...
	private static void deplacementBas(int i, int j, int m, String[] JR, String[][][] M, Tdonnee[] donnees)
			throws IOException // fct compiler
	{
		String c;
		if (M[m][i + 1][j].equals("0")) {
	...
	private static void collisionBas(int i, int j, int m, String[] JR, String[][][] M, Tdonnee[] donnees)
			throws IOException // fct compiler
	{
		if (M[m][i + 1][j].equals("m")) {
			elim(i, j, m, JR, M, donnees);
	...
	private static void deplacementGauche(int i, int j, int m, String[] JR, String[][][] M, Tdonnee[] donnees)
			throws IOException // fct compiler
	{
		String c;
	...
	private static void collisionGauche(int i, int j, int m, String[] JR, String[][][] M, Tdonnee[] donnees)
			throws IOException // fct compiler
	{
	...
	private static void deplacementDroite(int i, int j, int m, String[] JR, String[][][] M, Tdonnee[] donnees)
			throws IOException // fct compiler
	{
		String c;
		if (M[m][i][j + 1].equals("0")) {
	...
	private static void collisionDroite(int i, int j, int m, String[] JR, String[][][] M, Tdonnee[] donnees)
			throws IOException // fct compiler
	{
		if (M[m][i][j + 1].equals("m")) {
			elim(i, j, m, JR, M, donnees);
	...
	private static void choixTp(int m, int i, int j, String[] JR, String[][][] M, Tdonnee[] donnees, boolean cond1,
			boolean cond2) throws IOException// fct compile
	{
		int dir;
		if ((cond1) && (cond2)) {
	...
	private static void TP(int m, int i, int j, int sens, String[] JR, String[][][] M, Tdonnee[] donnees)
			throws IOException // fct compile
	{
	...
	private static void tourDeJeu(int[][] LP, String[] JR, String[][][] Mj, String[][][] Ms, Tdonnee[] donnees)
			throws IOException {
		...
		if (nDansEre > 0) {
			...
			iactif = LP[pactif][0];
			jactif = LP[pactif][1];
			System.out.println("Votre pion " + JR[jr] + " numero " + Mj[factif][iactif][jactif].substring(1, 2)
					+ " est defini comme votre pion actif ");
			action = 2; // action restante
			while ((action > 0) && (Mj[factif][iactif][jactif] != "0") && (Partie)) {
				z = 3 - action;// numero de l action en cours
				if (z == 2) {
					System.out.println("Le pion actif est le pion " + JR[jr] + " numero "
							+ Mj[factif][iactif][jactif].substring(1, 2));
				}
				...
				if (factif != 0) {
					cond2 = ((Mj[factif - 1][iactif][jactif] == "0") && (donnees[jr].res > 0));// Tp ds ere precedante
																								// possible
				} else {
					cond2 = false;
				}
	...
	private static String[][] lireMatrice(String fichier, String epoque) throws IOException {
		...
		while ((n = fileReader.read(Stock)) != -1)// je lis les donnes du fichier lorsqu'il n y a plus de caractere a
													// lire dans le fichier read() retourne -1
		{
			for (j = 0; j < n; j++)// j ajoute les caractere mu dans Stock, pour former la ligne
			{
				...
				if (Stock[j] == '\n') // si je saute une ligne alors il n y a plus de caractere a lire
				{
					...
					if (i < 6 && !ligne.startsWith("Etat de la matrice") && !ligne.contains("---")) // ca permet de
																									// prendre juste les
																									// chiffres
					{
						M[i] = ligne.split("\\s+"); // je rempli la matrice que je vais renvoyer, split ca permet de
													// diviser la ligne lu en espaces
						i += 1;
					}
					...
					if (ligne.contains("joueur 0")) {
						...
						if (ligne.contains("joueur 1")) {
							...
							if (ligne.contains("Reserve")) {
								if (cjoueur != -1) {
									reserve = Integer.parseInt(ligne.split(":")[1].trim());// en gros ca permet de
																							// prendre les chiffres qui
																							// sont apres : et de les
																							// convertir en int plus
																							// precisement le 1 indique
																							// que l on prend les
																							// caracteres apres :
									if (cjoueur == 0) {
	...
	private static void save(String Sauvegarde, String[][][] M, String NomInterieur, Tdonnee[] donnees)
			throws IOException {
		String caractere, ress, focs, nbrpas, nbrprs, nbrfus, nbreres, ligne;
		int m, i, k, j;

		File saveFile = new File(Sauvegarde);
		if (saveFile.exists()) { // on transforme le string en fichier
			saveFile.delete();
			ajouterLigne(Sauvegarde, NomInterieur);
			for (m = 0; m < 3; m++) {
				ajouterLigne(Sauvegarde,
						"Etat de la matrice" + (m == 0 ? "Passe" : m == 1 ? "Present" : "Futur") + " :");
				ajouterLigne(Sauvegarde, "----------------------------------------------");
				for (i = 0; i < 6; i++) {
					ligne = " ";
					for (j = 0; j < 6; j++) {
	...
	private static void menu(int[][] LP, String[] JR, String[][][] Mj, String[][][] Ms, Tdonnee[] donnees)
			throws IOException {
		int choix;
		String fichier;
		affichageMenu();
	...
	private static String choixSauvegarde(int fct, int[][] LP, String[] JR, String[][][] Mj, String[][][] Ms,
			Tdonnee[] donnees) throws IOException {
		int choix;
		String sauvegarde;
	...
	private static void jeu(int[][] LP, String[] JR, String[][][] Mj, String[][][] Ms, Tdonnee[] donnees)
			throws IOException {
		Partie = true;
		while (Partie) {
	...
	private static void reinitialisation(int[][] LP, String[] JR, String[][][] Mj, String[][][] Ms, Tdonnee[] donnees)
			throws IOException {
		int choix;
		affichageReinitialisation();
		System.out.println(
				"attention vous allez reinitialiser le jeu et toutes vos donnees sauvegardees seront supprimees");
